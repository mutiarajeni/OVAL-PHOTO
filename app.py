...
@app.route("/")
def beranda():
    return render_template("user/beranda.htmll")
...
def katalog_layanan():
    ...
    return render_template(
        "user/katalog_layanan.html", layanan=layanan_list, current_route=request.path
    )
    ...
    try:
        ...
        for item in gallery_items_cursor:
            ...
            if item.get("kategori") == "layanan" and item.get("id_layanan"):
                try:
                    layanan_obj = db.layanan.find_one(
                        {"_id": ObjectId(item["id_layanan"])}
                    )
                    if layanan_obj:
                        nama_layanan_terkait = layanan_obj.get("nama", "N/A")
                except Exception as e:
                    logger.warning(
                        f"Error ObjectId/query layanan untuk galeri {item.get('_id')}: {e}"
                    )

            elif item.get("kategori") == "lokasi" and item.get("id_lokasi"):
                try:
                    lokasi_obj = db.lokasi.find_one(
                        {"_id": ObjectId(item.get("id_lokasi"))}
                    )
                    if lokasi_obj:
                        nama_lokasi_terkait = lokasi_obj.get("nama", "N/A")
                except Exception as e:
                    logger.warning(
                        f"Error ObjectId/query lokasi untuk galeri {item.get('_id')}: {e}"
                    )
...
def admin_galeri_toggle_status(item_id):
    ...
    try:
        ...
        result = db.galeri.update_one(
            {"_id": ObjectId(item_id)}, {"$set": {"status": status_baru}}
        )

        if result.matched_count == 0:
            return (
                jsonify({"success": False, "message": "Item galeri tidak ditemukan."}),
                404,
            )
        ...
        return (
            jsonify({"success": False, "message": "Terjadi kesalahan internal."}),
            500,
        )
    ...
    if request.method == "POST":
        ...
        if gambar_files:
            for i, f in enumerate(gambar_files):
                app.logger.debug(
                    f"File {i}: filename='{f.filename}', content_type='{f.content_type}'"
                )
        ...
        if not os.path.exists(base_save_path):
            try:
                ...
                flash(
                    "Terjadi kesalahan server saat membuat direktori penyimpanan.",
                    "danger",
                )
                return redirect(url_for("admin_galeri_tambah"))
        ...
        for gambar in gambar_files:
            if gambar and gambar.filename:
                ...
                app.logger.warning(
                    "Sebuah file dilewati karena tidak valid atau tidak ada filename."
                )

        if not nama_file_gambar:  # Jika tidak ada file yang berhasil disimpan
            flash(
                "Tidak ada file gambar yang berhasil diproses atau disimpan.", "warning"
            )
            return redirect(url_for("admin_galeri_tambah"))
...
def admin_galeri_ubah(item_id):
    ...
    if not os.path.exists(upload_folder_galeri):
        try:
            ...
            logger.error(
                f"Gagal membuat direktori upload '{upload_folder_galeri}': {e}",
                exc_info=True,
            )
            flash(
                f"Gagal membuat direktori upload: {e}. Periksa izin folder.", "danger"
            )
            return redirect(url_for("admin_galeri"))
    ...
    try:
        ...
        if not gallery_item_to_edit:
            logger.error(
                f"Item galeri dengan ID '{item_id}' tidak ditemukan di database."
            )
        ...
        if request.method == "POST":
            logger.info(
                f"Menerima POST request untuk ubah gambar galeri ID: '{item_id}'"
            )
                ...
                if existing_filename in deleted_images_filenames:
                    try:
                        file_path_to_delete = os.path.join(
                            upload_folder_galeri, existing_filename
                        )
                        if os.path.exists(file_path_to_delete):
                            os.remove(file_path_to_delete)
                            logger.info(
                                f"File lama berhasil dihapus dari disk: '{existing_filename}'"
                            )
                        else:
                            logger.warning(
                                f"File lama '{existing_filename}' tidak ditemukan di disk saat mencoba menghapus. Mungkin sudah dihapus sebelumnya."
                            )
                    except Exception as e_del:
                        logger.error(
                            f"ERROR: Gagal menghapus file fisik '{existing_filename}': {e_del}",
                            exc_info=True,
                        )
                        flash(
                            f"Gagal menghapus beberapa gambar lama: {existing_filename}. Detail: {e_del}",
                            "warning",
                        )
                else:
            ...
            newly_saved_filenames = (
                []
            )  # List untuk menyimpan nama file baru yang berhasil disimpan
            ...
            for new_file in valid_new_files:
                ...
                if not file_ext or file_ext.lower() not in allowed_extensions:
                    logger.warning(
                        f"File '{original_filename}' dilewati: Ekstensi tidak valid atau tidak ada ('{file_ext}')."
                    )
                    flash(
                        f"File '{original_filename}' dilewati karena ekstensi tidak valid.",
                        "warning",
                    )
                    continue
                ...
                try:
                    logger.debug(
                        f"Mencoba menyimpan file: '{original_filename}' sebagai '{unique_filename}' ke '{file_path_to_save}'"
                    )
                    new_file.save(file_path_to_save)
                    newly_saved_filenames.append(unique_filename)
                    logger.info(
                        f"File baru berhasil disimpan ke disk: '{unique_filename}'"
                    )
                except Exception as e_save:
                    logger.error(
                        f"ERROR: Gagal menyimpan file baru '{unique_filename}' ke '{file_path_to_save}': {e_save}",
                        exc_info=True,
                    )
                    flash(
                        f"Gagal menyimpan file baru '{original_filename}'. Detail: {e_save}. Periksa izin folder.",
                        "danger",
                    )
            ...
            logger.info(
                f"Item galeri ID '{item_id}' berhasil diupdate di database. Total gambar sekarang: {len(updated_image_list)}."
            )
        ...
        logger.error(
            f"Terjadi error tak terduga di route /admin_galeri_ubah untuk ID '{item_id}': {e}",
            exc_info=True,
        )
...
def get_galeri_data():
    ...
    app.logger.debug(
        f"API call received: kategori='{kategori}', pilihan_kategori_id='{pilihan_kategori_id}'"
    )
    ...
    try:
        if kategori and kategori != "all":
            ...
            if pilihan_kategori_id and pilihan_kategori_id != "all":
                app.logger.debug(
                    f"Applying specific filter: kategori='{kategori}', ID='{pilihan_kategori_id}'"
                )
                if kategori == "layanan":
                    try:
                        ...
                        if layanan_obj:
                            description_data["nama_layanan"] = layanan_obj.get(
                                "nama", "N/A"
                            )
                            # Mengirim biaya dalam bentuk raw (string/angka) ke frontend
                            description_data["biaya"] = layanan_obj.get("harga", "N/A")
                        app.logger.debug(
                            f"Layanan object found: {layanan_obj.get('nama')} with ID {obj_id}"
                        )
                    except Exception as e:
                        app.logger.error(
                            f"Error converting layanan_id to ObjectId or finding layanan: {e}",
                            exc_info=True,
                        )
                        description_data = {
                            "nama_layanan": "-",
                    ...
                    try:
                        ...
                        if lokasi_obj:
                            ...
                            description_data["maps"] = lokasi_obj.get(
                                "link_maps", "N/A"
                            )
                            # Mengirim biaya dalam bentuk raw (string/angka) ke frontend
                            description_data["biaya"] = lokasi_obj.get("biaya", "N/A")
                        app.logger.debug(
                            f"Lokasi object found: {lokasi_obj.get('nama')} with ID {obj_id}"
                        )
                    except Exception as e:
                        app.logger.error(
                            f"Error converting lokasi_id to ObjectId or finding lokasi: {e}",
                            exc_info=True,
                        )
                        description_data = {
                            "nama_layanan": "-",
                ...
                app.logger.debug(
                    f"Kategori selected, but specific choice is 'all'. Query: {query}"
                )
        else:
            description_data = {
                "nama_layanan": "-",
    ...
    return jsonify(
        {"gallery_items": galeri_items, "description_data": description_data}
    )
...
def admin_pesanan():
    ...
    return render_template(
        "admin/pesanan.html", pesanan=pesanan, current_route=request.path
    )
...
def admin_pesanan_tambah():
    if request.method == "POST":
        try:
            ...
            biaya_transportasi = (
                int(biaya_transportasi_str.replace(".", "").replace(",", ""))
                if biaya_transportasi_str.replace(".", "").replace(",", "").isdigit()
                else 0
            )
            ...
            tanggal_selesai_acara = datetime.strptime(
                tanggal_selesai_acara_str, "%Y-%m-%d"
            )
            ...
            if lokasi_pilihan_user == "pilih_lokasi_sendiri":
                ...
                selected_lokasi = db.lokasi.find_one(
                    {"_id": ObjectId(lokasi_pilihan_user)}
                )
                if selected_lokasi:
            ...
            recalculated_total_harga = (
                harga_paket + biaya_tambahan_hari + biaya_lokasi + biaya_transportasi
            )
            ...
            if "surat_izin_lokasi" in request.files:
                ...
                if surat_izin_file and surat_izin_file.filename != "":
                    ...
                    file_path = os.path.join(
                        app.config["UPLOAD_FOLDER_SURAT_IZIN"], unique_filename
                    )
            ...
            )  # Redirect to admin pesanan list after successful booking
            ...
            return render_template(
                ...
                selected_paket_harga_raw=float(
                    request.form.get("harga_paket_dasar", "0")
                ),
                selected_paket_deposit_raw=float(
                    request.form.get("deposit_paket_dasar", "0")
                ),
            )
    ...
    for lokasi in lokasi_list:
        lokasi["_id"] = str(lokasi["_id"])
    return render_template(
        "admin/pesanan_tambah.html", layanan_list=layanan_list, lokasi_list=lokasi_list
    )
...
def admin_pesanan_detail():
    ...
    try:
        ...
        return render_template(
            "admin/pesanan_detail.html", pesanan=[pesanan], current_route=request.path
        )
...
def admin_pesanan_update(pesanan_id):
    ...
    biaya_transportasi_str = (
        request.form.get("biaya_transportasi", "0").replace(".", "").replace(",", "")
    )
    try:
...
def ripe_menunggu_konfirmasi():
    ...
    try:
        ...
        return render_template(
            "user/ripe_menunggu_konfirmasi.html",
            pesanan=all_pesanan_list,
            current_route=request.path,
        )
        ...
        # atau redirect ke halaman dashboard admin.
        return render_template(
            "user/ripe_menunggu_konfirmasi.html", pesanan=[], current_route=request.path
        )
...
def admin_timFotografi_ubah(tim_id):
    ...
    if request.method == "POST":
        ...
        existing_tim_with_name = db.tim.find_one(
            {
                ...
                "_id": {
                    "$ne": ObjectId(tim_id)
                },  # Pastikan bukan tim yang sedang diubah
            }
        )
        ...
        if existing_tim_with_name:
            ...
            return render_template(
                "admin/timFotografi_ubah.html", tim=tim, tim_exists=True
            )
        ...
        if gambar_baru and allowed_file(gambar_baru.filename):
            ...
            if nama_file_gambar and nama_file_gambar != secure_filename(
                gambar_baru.filename
            ):
                old_image_path = os.path.join(
                    app.config["UPLOAD_FOLDER"], nama_file_gambar
                )
                if os.path.exists(old_image_path):
                    os.remove(old_image_path)
...
def admin_faq_tambah():
    ...
    if request.method == "POST":
        ...
        if faq_sudah_ada:
            ...
            return render_template(
                "admin/faq_tambah.html",
                error_message="Pertanyaan ini sudah ada.",
                data_input=request.form,
            )
...
def admin_faq_ubah(id_faq):
    ...
    try:
        ...
        if not faq_untuk_diubah:
            flash(
                "FAQ tidak ditemukan.", "danger"
            )  # Flash masih bisa digunakan untuk error penting
            return redirect(url_for("admin_faq"))
    except Exception as e:
    ...
    if request.method == "POST":
        ...
        pertanyaan_baru = request.form[
            "pertanyaan"
        ].strip()  # Admin bisa mengedit pertanyaan juga

        if not pertanyaan_baru or not jawaban_baru:
            return render_template(
                "admin/faq_ubah.html",
                faq=faq_untuk_diubah,
                error_message="Pertanyaan dan jawaban tidak boleh kosong.",
            )
...
def form_faq_user():
    ...
    if request.method == "POST":
        ...
        pertanyaan = request.form.get(
            "pertanyaan_user"
        )  # <--- KONSISTENSI NAMA FIELD DENGAN HTML

        print(
            f"Data form diterima: Nama='{nama_pengaju}', Email='{email_pengaju}', Pertanyaan='{pertanyaan}'"
        )
        ...
        try:
            ...
            return redirect(
                url_for("faq_user")
            )  # Redirect ke halaman FAQ utama user setelah berhasil
        except Exception as e:
...
def api_daftar():
    ...
    if password != konfirpassword:
        return (
            jsonify(
                {"success": False, "message": "Konfirmasi kata sandi tidak cocok."}
            ),
            400,
        )
    ...
    if len(password) < 6:
        return (
            jsonify(
                {"success": False, "message": "Kata sandi harus minimal 6 karakter."}
            ),
            400,
        )
    ...
    if users_collection.find_one({"username": username}):
        return (
            jsonify({"success": False, "message": "Nama pengguna sudah terdaftar."}),
            409,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {"success": True, "message": "Pendaftaran berhasil! Silakan masuk."}
            ),
            201,
        )
    except Exception as e:
...
def api_reset_password():
    ...
    try:
        ...
        if not user:
            return (
                jsonify({"success": False, "message": "Pengguna tidak ditemukan."}),
                404,
            )
        ...
        users_collection.update_one(
            {"email": email}, {"$set": {"password_hash": hashed_new_password}}
        )
        return (
            jsonify(
                {
        ...
        return (
            jsonify(
                {"success": False, "message": "Tautan reset password tidak valid."}
            ),
            401,
        )
    except Exception as e:
...
def admin_akunKlien():
    ...
    print(
        f"DEBUG: Jumlah user yang ditemukan di database: {len(users_data)}"
    )  # --- TAMBAHKAN INI ---
    if users_data:
        print(
            f"DEBUG: Contoh data user pertama: {users_data[0]}"
        )  # --- TAMBAHKAN INI ---
    else:
    ...
    return render_template(
        "admin/akunKlien.html", users=users_data, current_route=request.path
    )
...
if __name__ == "__main__":
    ...
    if (
        app.config["SECRET_KEY"] == "DANANDA34"
        or app.config["JWT_SECRET_KEY"] == "KILLING23"
    ):
        print(
            "WARNING: Using default secret keys. Please set FLASK_SECRET_KEY and JWT_SECRET_KEY environment variables in production for security."
        )
